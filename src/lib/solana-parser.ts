...
export class SolanaTransactionParser {
  ...
  private extractTokenTransfers(transaction: any, walletAddress: string): TokenTransfer[] {
    ...
    try {
      ...
      for (let i = 0; i < preTokenBalances.length; i++) {
        ...
        const account = accountKeys[pre.accountIndex];
        
        // Skip if we can't verify ownership
        if (!account) continue;
      ...
      for (let i = 0; i < accountKeys.length; i++) {
        ...
        if (typeof account === 'string' && account === walletAddress) {
          const preBalance = preBalances[i] || 0;
          const postBalance = postBalances[i] || 0;
          const difference = postBalance - preBalance;
          
          // Exclude transaction fees by checking significant balance changes
          if (Math.abs(difference) > 100000) { // More than 0.0001 SOL
            transfers.push({
  ...
  private identifyDEX(transaction: any): string {
    ...
    const programIds = accountKeys
      .map((key: any) => typeof key === 'string' ? key : key.pubkey)
    ...
    const dexPrograms: Record<string, string> = {
      ...
      'EUqojwWA2rd19FZrzeBncJsm38Jm1hEhE3zsmX3bRc2o': 'lifinity'
    };
